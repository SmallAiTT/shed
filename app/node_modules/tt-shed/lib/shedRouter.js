...
router.post("/shed/comp/children", function(req, res, next){
    ...
    var sql = "select b.*, a.count from (select childId, count from compDtl where id = ?) a left join comp b on a.childId = b.id ";
    req.dbClient.pageGuideSqlByReq(sql, [parentId], req, {code:"=?"}, function(err, data){
        if(err) return next(err);
        res.send(data);
    })
});
