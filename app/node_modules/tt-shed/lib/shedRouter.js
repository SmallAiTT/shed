var express = require("express");
var router = module.exports = express.Router();
var async = require("async");
var path = require("path");

function getCompByReq(req){
    var body = req.body;
    return {
        id : body.id,
        code : body.code,
        name : body.name,
        isBase : body.isBase,
        img : body.img,
        spec : body.spec,
        mates : body.mates,
        unit : body.unit,
        remark : body.remark
    }
}

//++++++++++++++++++++++shed 开始+++++++++++++++++++++++++++++++++
//++++++++++++++++++++++shed 结束+++++++++++++++++++++++++++++++++

//++++++++++++++++++++++proj 开始+++++++++++++++++++++++++++++++++
router.get("/shed/proj_mgr", function(req, res){
    res.render("shed/proj_mgr");
});
function getProjByReq(req){
    var body = req.body;
    return {
        id : body.id,
        name : body.name,
        state : body.state,
        remark : body.remark
    }
}
router.post("/shed/proj/list", function(req, res, next){
    req.dbClient.pageGridByReq("proj", {_orderBy : ["name"]}, req, {name : "like ?"}, function(err, data){
        if(err) return next(err);
        res.send(data);
    });
});
router.post("/shed/proj/create", function(req, res, next){
    var proj = getProjByReq(req);
    proj.id = null;
    proj.state = proj.state || "000";
    req.dbClient.insert("proj", proj, function(err){
        if(err) return next(err);
        res.success();
    });
});
router.post("/shed/proj/update", function(req, res, next){
    var proj1 = getProjByReq(req);
    req.dbClient.select("proj", {id : proj1.id}, function(err, proj){
        if(err) return next(err);
        if(proj.state == "001") return res.error("已经完成的项目不可编辑！");

        proj.name = proj1.name;
        proj.remark = proj1.remark;
        req.dbClient.update("proj",proj , {id : proj.id}, function(err1){
            if(err1) return next(err1);
            res.success();
        });
    });
});
router.post("/shed/proj/delete", function(req, res, next){
    var proj = getProjByReq(req);
    req.dbClient.select("proj", {id : proj.id}, function(err, proj){
        if(err) return next(err);
        if(proj.state == "001") return res.error("已经完成的项目不可删除！");

        req.dbClient.del("proj", {id : proj.id}, function(err1){
            if(err1) return next(err1);
            res.success();
        });
    });
});
router.post("/shed/proj/finish", function(req, res, next){
    var proj = getProjByReq(req);
    req.dbClient.update("proj",{state : "001"} , {id : proj.id}, function(err){
        if(err) return next(err);
        res.success();
    });
});
router.post("/shed/proj/calComp", function(req, res, next){
    var projId = req.body.id;
    req.dbClient.list("shed", {projId : projId}, function(err, sheds){
        async.map(sheds, function(shed, cb1){
            calByShed(req.dbClient, shed, cb1);
        }, function(err, results){
            if(err) return next(err);

            var calList = [];
            var calMap = {};
            for(var i = 0; i < results.length; ++i){
                var shedCalList = results[i];
                for(var j = 0; j < shedCalList.length; ++j){
                    var compInfo = shedCalList[j];
                    var compId = compInfo.id;
                    var info = calMap[compId];
                    if(info){
                        info.count += compInfo.count;
                    }else{
                        calMap[compId] = compInfo;
                    }
                }
            }
            for(var compId in calMap){
                calList.push(calMap[compId]);
            }
            res.send(calList);
        });
    });
});
//++++++++++++++++++++++proj 结束+++++++++++++++++++++++++++++++++

//++++++++++++++++++++++exp 开始+++++++++++++++++++++++++++++++++
//通过req获取公式对象
function getExpByReq(req){
    var body = req.body;
    return {
        id : body.id,
        name : body.name,
        exp : body.exp,
        remark : body.remark
    }
};
router.get("/shed/exp_mgr", function(req, res){
    res.render("shed/exp_mgr");
});
router.post("/shed/exp/list", function(req, res, next){
    req.dbClient.pageGridByReq("exp", {_orderBy : ["name"]}, req, {name:"like ?"}, function(err, data){
        if(err) return next(err);
        res.send(data);
    });
});
//新增零件
router.post("/shed/exp/create", function(req, res, next){
    var exp = getExpByReq(req);
    exp.id = null;
    req.dbClient.insert("exp", exp, function(err){
        if(err) return next(err);
        res.success();
    });
});
//修改零件
router.post("/shed/exp/update", function(req, res, next){
    var exp = getExpByReq(req);
    req.dbClient.update("exp",exp , {id : exp.id}, function(err){
        if(err) return next(err);
        res.success();
    });
});
//删除零件
router.post("/shed/exp/delete", function(req, res, next){
    var id = req.body.id;
    //TODO 这里需已经关联的公式进行判断，如果已经进行关联了，就不能删除。
    req.dbClient.del("exp", {id : id}, function(err1){//再删除零件本身
        if(err1) return next(err1);
        res.success();
    });
});
//++++++++++++++++++++++exp 结束+++++++++++++++++++++++++++++++++

//++++++++++++++++++++++sche 开始+++++++++++++++++++++++++++++++++
//通过req获取到方案数据
function getScheByReq(req){
    var body = req.body;
    return {
        id : body.id,
        name : body.name,
        remark : body.remark
    }
};
router.get("/shed/sche_mgr", function(req, res){
    res.render("shed/sche_mgr");
});
router.post("/shed/sche/list", function(req, res, next){
    req.dbClient.pageGridByReq("sche", {_orderBy : ["name"]}, req, {name:"like ?"}, function(err, data){
        if(err) return next(err);
        res.send(data);
    });
});
//新增零件
router.post("/shed/sche/create", function(req, res, next){
    var sche = getScheByReq(req);
    sche.id = null;
    req.dbClient.insert("sche", sche, function(err){
        if(err) return next(err);
        res.success();
    });
});
//修改零件
router.post("/shed/sche/update", function(req, res, next){
    var sche = getScheByReq(req);
    req.dbClient.update("sche",sche , {id : sche.id}, function(err){
        if(err) return next(err);
        res.success();
    });
});
//删除零件
router.post("/shed/sche/delete", function(req, res, next){
    var id = req.body.id;
    //TODO 这里需已经关联的公式进行判断，如果已经进行关联了，就不能删除。
    req.dbClient.del("sche", {id : id}, function(err1){//再删除零件本身
        if(err1) return next(err1);
        res.success();
    });
});

//删除零件
router.post("/shed/sche/test", function(req, res, next){
    var id = req.body.id;
    var shed = getShedByReq(req);
    shed.scheId = id;//设置方案id
    shed.count = 1;//设置成一个
    calByShed(req.dbClient, shed, function(err, results){
        if(err) return next(err);
        res.send(results);
    });
});
//将字符串转成int数组
function strToIntArr(str){
    var arr = str.split(",");
    var results = [];
    for(var i = 0; i < arr.length; ++i){
        results.push(parseInt(arr[i]));
    }
    return results;
}
//初始化shed数据
function initShed(shed){
    shed.count = shed.count || 1;
    if(typeof shed.lengths == "string") shed.lengths = strToIntArr(shed.lengths);
    if(typeof shed.topScrolls == "string") shed.topScrolls = strToIntArr(shed.topScrolls);
    if(typeof shed.edgeScrolls == "string") shed.edgeScrolls = strToIntArr(shed.edgeScrolls);
    if(typeof shed.frontScrolls == "string") shed.frontScrolls = strToIntArr(shed.frontScrolls);
}
//通过shed配置数据，计算出零件列表
function calByShed(client, shed, cb){
    initShed(shed);
    var scheId = shed.scheId;
    async.parallel([
        function(cb1){
            //获取到零件公式内容列表
            var sqlStr = "select e.exp, s.name scheName, cex.* from sche s, compexp cex, exp e where s.id = ? and s.id = cex.scheId and cex.expId = e.id order by cex.seq";
            client.query(sqlStr, [scheId], cb1);
        },
        function(cb1){
            //获取到相关零件列表
            var sqlStr = "select c.name, c.code, c.isBase, c.mates, c.spec, c.unit, s.name scheName, cex.* from sche s, compexp cex, comp c where s.id = ? and s.id = cex.scheId and cex.compId = c.id";
            client.query(sqlStr, [scheId], function(err, list){
                if(err) return cb1(err);
                var compMap = {};
                for(var i = 0; i < list.length; ++i){
                    var comp = list[i];
                    if(compMap[comp.compId]) continue;
                    compMap[comp.compId] = comp;
                }
                cb1(null, compMap);
            });
        },
        function(cb1){
            //获取到有关联的子零件列表
            var sqlStr = "select c.id parentId, c2.id compId, cd.count, c2.* from sche s, compexp cex, comp c, compdtl cd, comp c2 where s.id = ? and s.id = cex.scheId and cex.compId = c.id and c.isBase != 1 and cd.id = c.id and c2.id = cd.childId";
            client.query(sqlStr, [scheId], function(err, list){
                if(err) return cb1(err);
                var dtlMap = {};//零件组的列表映射
                for(var i = 0; i < list.length; ++i){
                    var dtl = list[i];
                    var parentId = dtl.parentId;
                    var dtlList = dtlMap[parentId];
                    if(!dtlList){
                        dtlList = dtlMap[parentId] = [];
                    }
                    dtlList.push(dtl);
                }
                cb1(null, dtlMap);
            });
        }
    ], function(err, results){
        if(err) return cb(err);
        var expInfoList = results[0];//公式列表（已经按照seq进行了排序）
        var compMap = results[1];//公式配置中对应的零件映射
        var dtlMap = results[2];//零件组映射

        var result = {};
        var compInfoMap = {};//存储零件信息的映射
        try{
            for(var i = 0; i < expInfoList.length; ++i){
                var info = expInfoList[i];
                var exp = info.exp;
                var compId = info.compId;
                var comp = compMap[compId];
                if(!compInfoMap[compId]) {
                    compInfoMap[compId] = {
                        id : compId,
                        code : comp.code,
                        name : comp.name,
                        isBase : comp.isBase,
                        spec : comp.spec,
                        mates : comp.mates,
                        unit : comp.unit,
                        remark : comp.remark
                    }
                }
                exp = eval('(' + exp + ')');//得到公式function对象
                var value = shed.count * exp(shed.lengths, shed.topScrolls, shed.edgeScrolls, shed.frontScrolls)
                result[compId] = (result[compId] || 0) + value;
                if(!comp.isBase){//如果是零件组，则需要算出其子零件数量
                    var dtlList = dtlMap[compId] || [];
                    for(var j = 0; j < dtlList.length; ++j){
                        var dtl = dtlList[j];
                        var dtlCompId = dtl.compId;
                        if(!compInfoMap[dtlCompId]) {//初始化数据
                            console.log(dtl.name);
                            compInfoMap[dtlCompId] = {
                                id : dtlCompId,
                                code : dtl.code,
                                name : dtl.name,
                                isBase : dtl.isBase,
                                spec : dtl.spec,
                                mates : dtl.mates,
                                unit : dtl.unit,
                                remark : dtl.remark
                            }
                        }
                        result[dtlCompId] = (result[dtlCompId] || 0) + value * dtl.count;
                    }
                }
            }

            var calList = [];
            for(var compId in result){
                var compInfo = compInfoMap[compId];
                if(!compInfo.isBase) continue;//非基础零件则不算在内
                compInfo.count = result[compId];
                calList.push(compInfo);
            }
            cb(null, calList);
        }catch(e){
            return cb(e);
        }
    });
}

//++++++++++++++++++++++sche 结束+++++++++++++++++++++++++++++++++


//++++++++++++++++++++++compExp 开始+++++++++++++++++++++++++++++++++
//通过req获取到零件公式配置
function getCompExpByReq(req){
    var body = req.body;
    return {
        id : body.id,
        scheId : body.scheId,
        compId : body.compId,
        expId : body.expId,
        seq : body.seq,
        args : body.args,
        remark : body.remark
    }
};
router.post("/shed/compExp/list", function(req, res, next){
    var scheId = req.body.scheId;
    var sqlStr = "select a.*, e.name expName, c.code compCode, c.name compName from compexp a, exp e, comp c where a.scheId = ? and a.expId = e.id and a.compId = c.id order by a.seq";
    req.dbClient.pageGridSqlByReq(sqlStr, [scheId], req, {seq:"=?", compCode:"like ?",  compName:"like ?"}, function(err, data){
        if(err) return next(err);
        res.send(data);
    });
});
//新增零件
router.post("/shed/compExp/create", function(req, res, next){
    var compExp = getCompExpByReq(req);
    compExp.id = null;
    req.dbClient.insert("compExp", compExp, function(err){
        if(err) return next(err);
        res.success();
    });
});
//修改零件
router.post("/shed/compExp/update", function(req, res, next){
    var compExp = getCompExpByReq(req);
    req.dbClient.update("compExp",compExp , {id : compExp.id}, function(err){
        if(err) return next(err);
        res.success();
    });
});
//删除零件
router.post("/shed/compExp/delete", function(req, res, next){
    var id = req.body.id;
    //TODO 这里需已经关联的公式进行判断，如果已经进行关联了，就不能删除。
    req.dbClient.del("compExp", {id : id}, function(err1){//再删除零件本身
        if(err1) return next(err1);
        res.success();
    });
});
//++++++++++++++++++++++compExp 结束+++++++++++++++++++++++++++++++++


//++++++++++++++++++++++comp 开始+++++++++++++++++++++++++++++++++
//零件管理界面链接
router.get("/shed/comp_mgr", function(req, res){
    res.render("shed/comp_mgr");
});
//零件列表
router.post("/shed/comp/list", function(req, res, next){
    if(req.body.isBase == -1) req.body.isBase = null;
    req.dbClient.pageGridByReq("comp", {_orderBy : ["code", "name"]}, req, {code:"like ?", name:"like ?", isBase:"=?"}, function(err, data){
        if(err) return next(err);
        res.send(data);
    });
});
//新增零件
router.post("/shed/comp/create", function(req, res, next){
    var comp = getCompByReq(req);
    comp.id = null;
    req.dbClient.insert("comp", comp, function(err){
        if(err) return next(err);
        res.success();
    });
});
//修改零件
router.post("/shed/comp/update", function(req, res, next){
    var comp = getCompByReq(req);
    req.dbClient.update("comp",comp , {id : comp.id}, function(err){
        if(err) return next(err);
        res.success();
    });
});
//删除零件
router.post("/shed/comp/delete", function(req, res, next){
    var id = req.body.id;
    req.dbClient.del("compDtl", "id = ? or childId = ?", [id, id], function(err){//先删除子零件相关
        if(err) return next(err);
        req.dbClient.del("comp", {id : id}, function(err1){//再删除零件本身
            if(err1) return next(err1);
            res.success();
        });
    });
});
//子零件列表
router.post("/shed/comp/children", function(req, res, next){
    var parentId = req.body.parentId;
    var sql = "select a.parentId, a.count, b.* from (select id parentId, childId, count from compDtl where id = ?) a left join comp b on a.childId = b.id ";
    req.dbClient.pageGridSqlByReq(sql, [parentId], req, {code:"like ?", name:"like ?"}, function(err, data){
        if(err) return next(err);
        res.send(data);
    })
});
//绑定子零件时的零件列表
router.post("/shed/comp/compToBeSelected", function(req, res, next){
    var parentId = req.body.parentId;
    var currChildId = req.body.currChildId;
    var sql = "select * from comp where id != ? and isBase = 1 and id not in (select childId id from compdtl where id = ? ) order by code, name";
    req.dbClient.pageGridSqlByReq(sql, [parentId, currChildId], req, {code:"like ?", name:"like ?"}, function(err, data){
        if(err) return next(err);
        res.send(data);
    })
});
//绑定子零件时的零件列表
router.post("/shed/comp/compToBeSelected", function(req, res, next){
    var parentId = req.body.parentId;
    var currChildId = req.body.currChildId;
    var sql = "select * from comp where id != ? and isBase = 1 and id not in (select childId id from compdtl where id = ? ) order by code, name";
    req.dbClient.pageGridSqlByReq(sql, [parentId, currChildId], req, {code:"like ?", name:"like ?"}, function(err, data){
        if(err) return next(err);
        res.send(data);
    })
});
//通过req获取到零件映射信息
function getCompDtlByReq(req){
    var body = req.body;
    return {
        id : body.parentId,
        childId : body.id,
        count : body.count
    }
}
//添加子零件
router.post("/shed/comp/addChild", function(req, res, next){
    var compDtl = getCompDtlByReq(req);
    req.dbClient.insert("compDtl", compDtl, function(err){
        if(err) return next(err);
        res.success();
    })
});
//修改子零件
router.post("/shed/comp/updateChild", function(req, res, next){
    var body = req.body;
    var value = {childId : body.id, count : body.count};
    var cnd = {id : body.parentId, childId : body.oldId};
    req.dbClient.update("compDtl", value, cnd, function(err){
        if(err) return next(err);
        res.success();
    })
});
//删除子零件
router.post("/shed/comp/deleteChild", function(req, res, next){
    var body = req.body;
    req.dbClient.del("compDtl", {id : body.parentId, childId : body.id}, function(err){
        if(err) return next(err);
        res.success();
    })
});
//++++++++++++++++++++++comp 结束+++++++++++++++++++++++++++++++++


//++++++++++++++++++++++shed 开始++++++++++++++++++++++++++++++++++
//通过req获取到大棚配置信息
function getShedByReq(req){
    var body = req.body;
    return {
        id : body.id,
        name : body.name,
        projId : body.projId,
        scheId : body.scheId,
        count : body.count,
        lengths : body.lengths,
        topScrolls : body.topScrolls,
        edgeScrolls : body.edgeScrolls,
        frontScrolls : body.frontScrolls,
        remark : body.remark
    }
}
router.post("/shed/shed/list", function(req, res, next){
    var projId = req.body.projId;
    var sqlStr = "select shed.*, sche.name scheName from shed, sche where shed.scheId = sche.id and projId = ? order by name";
    req.dbClient.pageGridSqlByReq(sqlStr, [projId], req, {name:"like ?", scheName:"like ?"}, function(err, data){
        if(err) return next(err);
        res.send(data);
    })
});
router.post("/shed/shed/create", function(req, res, next){
    var shed = getShedByReq(req);
    shed.id = null;
    req.dbClient.select("proj", {id : shed.projId}, function(err, proj){
        if(err) return next(err);
        if(proj.state == "001") return res.error("已经完成的项目不能新增大棚！");

        req.dbClient.insert("shed", shed, function(err){
            if(err) return next(err);
            res.success();
        })
    });
});
router.post("/shed/shed/update", function(req, res, next){
    var shed = getShedByReq(req);
    req.dbClient.select("proj", {id : shed.projId}, function(err, proj){
        if(err) return next(err);
        if(proj.state == "001") return res.error("已经完成的项目不能修改大棚！");

        req.dbClient.update("shed", shed, {id : shed.id}, function(err){
            if(err) return next(err);
            res.success();
        })
    });
});
router.post("/shed/shed/delete", function(req, res, next){
    var shed = getShedByReq(req);
    req.dbClient.select("proj", {id : shed.projId}, function(err, proj){
        if(err) return next(err);
        if(proj.state == "001") return res.error("已经完成的项目不能删除大棚！");

        req.dbClient.del("shed", {id : shed.id}, function(err){
            if(err) return next(err);
            res.success();
        });
    });
});

router.post("/shed/shed/calComp", function(req, res, next){
    var id = req.body.id;
    req.dbClient.select("shed", {id:id}, function(err, shed){
        if(err) return next(err);
        if(!shed) return next("找不到shed数据，请检查！");
        req.dbClient.select("proj", {id:shed.projId}, function (err, proj){
            if(err) return next(err);
            if(!proj) return next("找不到proj数据，请检查！");
            calByShed(req.dbClient, shed, function(err, results){
                if(err) return next(err);
                exports.outputCompResults(proj.name, shed.name, results);
                res.send(results);
            });
        });
    });
});
//++++++++++++++++++++++shed 结束++++++++++++++++++++++++++++++++++

exports.outputCompResults = function(projName, shedName, results){
    /* require XLSX */
    var XLSX = require('XLSX')

    function datenum(v, date1904) {
        if(date1904) v+=1462;
        var epoch = Date.parse(v);
        return (epoch - new Date(Date.UTC(1899, 11, 30))) / (24 * 60 * 60 * 1000);
    }

    function sheet_from_array_of_arrays(data, opts) {
        var ws = {};
        var range = {s: {c:10000000, r:10000000}, e: {c:0, r:0 }};
        for(var R = 0; R != data.length; ++R) {
            for(var C = 0; C != data[R].length; ++C) {
                if(range.s.r > R) range.s.r = R;
                if(range.s.c > C) range.s.c = C;
                if(range.e.r < R) range.e.r = R;
                if(range.e.c < C) range.e.c = C;
                var cell = {v: data[R][C] };
                if(cell.v == null) continue;
                var cell_ref = XLSX.utils.encode_cell({c:C,r:R});

                if(typeof cell.v === 'number') cell.t = 'n';
                else if(typeof cell.v === 'boolean') cell.t = 'b';
                else if(cell.v instanceof Date) {
                    cell.t = 'n'; cell.z = XLSX.SSF._table[14];
                    cell.v = datenum(cell.v);
                }
                else cell.t = 's';

                ws[cell_ref] = cell;
            }
        }
        if(range.s.c < 10000000) ws['!ref'] = XLSX.utils.encode_range(range);
        return ws;
    }

    /* original data */
//    var data = [[1,2,3],[true, false, null, "sheetjs"],["foo","bar",new Date("2014-02-19T14:30Z"), "0.3"], ["baz", null, "qux"]]
    var data = [];
    data.push(["零件编码", "零件名称", "数量", "单位", "备注"]);
    for(var i = 0; i < results.length; ++i){
        var compInfo = results[i];
        data.push([compInfo.code, compInfo.name, compInfo.count, compInfo.unit, compInfo.remark]);
    }
    var ws_name = shedName;

    function Workbook() {
        if(!(this instanceof Workbook)) return new Workbook();
        this.SheetNames = [];
        this.Sheets = {};
    }

    var wb = new Workbook(), ws = sheet_from_array_of_arrays(data);

    /* add worksheet to workbook */
    wb.SheetNames.push(ws_name);
    wb.Sheets[ws_name] = ws;

    /* write file */
    var outputPath = path.join(__dirname, "../../../output", projName + '.xlsx')
    XLSX.writeFile(wb, outputPath);
};
//exports.outputCompResults();